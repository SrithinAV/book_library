    ...
    if(passwords.is_open()){
        ...
        while(getline(passwords, read_passwords)){
            ...
            if(password == read_passwords){
                ...
                cout<<"* * * * * *        * * * * * * *          * * * * *      * * * * * * * * * * * * * * *"<<endl;
                cout<<"*    1    *        *     2     *          *   3   *      *            4              *"<<endl;
                cout<<"*TAKE BOOK*        *RETURN BOOK*          *  EXIT *      * search book with alphabet * "<<endl;
                cout<<"* * * * * *        * * * * * * *          * * * * *      * * * * * * * * * * * * * * *"<<endl;
                cin>>choose;
                if(choose == 1){
                    system("cls");
   ...
   funalpha += alphabet_();
    fstream books;
                 ...
                 if(books.is_open())
                 {
                    ...
                    books.close();
                    cout<<"*********************************************"<<endl;
                     cout<<"SELECT BOOK (***Enter 'no' if you dont want***): "<<endl;
                     cin.ignore();
                     getline(cin, select_book);
                     select();
...
if(checking.is_open()){
 ...
 if(c==1){
  ...
  if(select_book != "no" && select_book != "search" && select_book != "Search" && select_book != "due_date" && select_book != "Due_date" && select_book !="o"){
